#include "iostream"
void move_position(const char* ptr) {
	int count = 0;
	int pos_x = 0;
	int pos_y = 0;
	while (*ptr != '\0') {
		...
		while (*ptr != ';') {
			...
			if (*ptr == 'A' || *ptr == 'S'|| *ptr == 'W'|| *ptr == 'D') {
				proper_latter += 1;
			}

			if ((*ptr<'0'||*ptr>'9')&&*ptr != 'A' && *ptr != 'S' && *ptr == 'W' && *ptr == 'D') {
				proper_latter += 2;
			}
			part_count++;
			ptr++;
		...
		if (proper_latter ==1) {
			const char* temptr = ptr -part_count;
			int mov_target = 0;
			if (part_count == 2) {
				mov_target = *(temptr + 1) - '0';
			}
			if (part_count == 3) {
				mov_target = 10*(*(temptr + 1) - '0')+ *(temptr + 2) - '0';
			}
			if (*temptr == 'A') {
				pos_x -= mov_target;
			}
